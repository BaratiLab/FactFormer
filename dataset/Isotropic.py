...
def initialize(solver, context, seed):
    c = context
...
if __name__ == "__main__":
    ...
    config.triplyperiodic.add_argument("--postfix", type=str)
    config.triplyperiodic.add_argument("--out_folder", type=str)
    config.triplyperiodic.add_argument("--random_seed", type=int, default=0)

    sol = get_solver(update=update, mesh="triplyperiodic")
    config.params.nu = (1./config.params.kd**(4./3.))
    os.makedirs(config.params.out_folder, exist_ok=True)
    context = sol.get_context()
    initialize(sol, context, config.params.random_seed)
    #init_from_file("NS_isotropic_60_60_60_c.h5", sol, context)
    context.hdf5file.filename = os.path.join(config.params.out_folder, f"NS_isotropic_{config.params.N[0]}_{config.params.N[0]}_{config.params.N[0]}_{config.params.postfix}")
